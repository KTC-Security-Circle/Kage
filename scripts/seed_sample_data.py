...
MEMO_SEEDS: tuple[MemoSeed, ...] = (
    ...
    MemoSeed(
        key="school_social_app",
        title="学内用SNSアプリを作成する",
        content="学内向けのSNSアプリを開発し、学生同士の交流を促進する。制作メンバーはセキュリティサークルのメンバー。まずはMVPを考えるところから。",
        status=MemoStatus.INBOX,
    ),
#     MemoSeed(
#         key="school_social_app_2",
#         title="学内SNSアプリ開発 - MVP計画メモ",
#         content="""# 学内向けSNSアプリ開発プロジェクト - MVP計画案

# 概要
# 目的: 学生同士の交流を促進し、コミュニティを活性化する学内限定のSNSアプリケーションを開発。

# 制作メンバー: セキュリティサークルのメンバー（セキュリティを最優先事項とする）。

# MVP（Minimum Viable Product）の要素
# MVPは、**「学内の特定のグループや活動に関する情報共有と交流」**に焦点を当て、セキュリティを担保した最小限の機能に絞る。

# 1. ユーザー認証とプロファイル
# 認証: 大学のメールアドレス（@university.ac.jpなど）による登録・認証を必須とする。これにより、学内ユーザー限定を確実にする。

# プロファイル: ニックネーム、学籍番号（非公開設定可）、所属学部/学科、サークル/研究室、自己紹介文のみ。個人情報の取り扱いに細心の注意を払う。

# 2. コミュニティ機能（限定的なグループ機能）
# グループ作成: サークルや研究室、授業など、特定のテーマに基づく**プライベートグループ（招待制/承認制）**の作成を可能とする。

# 投稿: グループ内でのテキスト投稿、画像のアップロード（セキュリティチェックを実装）。""",
#         status=MemoStatus.INBOX,
#     ),
)
