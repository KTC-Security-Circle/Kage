...
class NewMemoSection(ft.Column):
    """新規メモ作成セクションコンポーネント.

    新しいメモを作成するためのUIセクション。
    """

    def __init__(self, on_create_memo: Callable[[str], None]) -> None:
        """NewMemoSectionの初期化.

        Args:
            on_create_memo: メモ作成時のコールバック関数
        """
        super().__init__()
        self._on_create_memo = on_create_memo
    ...
    def _handle_create_click(self, _: ft.ControlEvent) -> None:
        """作成ボタンクリック処理
        ...
        if content and content.strip():
            self._on_create_memo(content.strip())
            self.content_field.value = ""  # [AI GENERATED] 入力欄をクリア
